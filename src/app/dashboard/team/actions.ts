...
/**
 * Sends the invitation email for a row that was already inserted into
 * lab_invites. The dashboard's invite modal calls this after its own
 * insert succeeds, passing the auto-generated token.
 *
 ...
 * in the Resend dashboard. Once verified, change the from address to
 * `invites@cagesync.com` (or similar).
 *
 * Returns { success, error? } — the caller can decide what to surface.
 * Failure is non-fatal: the invite row exists regardless, the PI can
 * always share the accept URL manually as a fallback.
 */
export async function sendInviteEmail(params: {
  ...
  try {
    ...
    // Re-fetch the invite (don't trust client-supplied params for the
    // payload that goes in the email).
